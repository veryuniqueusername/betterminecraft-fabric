...
public class BetterLeavesBlock extends LeavesBlock {
	private static final BooleanProperty EXPOSED = BooleanProperty.of("exposed");

	public BetterLeavesBlock(Settings properties) {
		super(properties);
		this.setDefaultState(this.getDefaultState().with(DISTANCE, 7).with(PERSISTENT, false).with(EXPOSED, true));
	}
	...
	public BlockState getPlacementState(ItemPlacementContext ctx) {
		...
		return getExposed(updateDistanceFromLogs(this.getDefaultState().with(PERSISTENT, true), world, pos), world, pos);
	}
